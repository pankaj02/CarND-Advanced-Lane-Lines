...
def warp_binary_pipeline(img):
    ...
    binary, _ = binary_gradient_image(undist)
...
def sliding_window_pipeline(img, visualize=False):
    ...
    if first_image == True:
        ...
        if visualize:
            visualize(binary_warped, leftx, lefty, rightx, righty, left_fit, right_fit)
        gleft_fit = left_fit
        ...
        if visualize:
            visualize_smoother(binary_warped, leftx, lefty, rightx, righty, left_fit, right_fit)

    return left_fit, right_fit, binary_warped, Minv


def visualize_smoother(binary_warped, leftx, lefty, rightx, righty, left_fit, right_fit):
...
def visualize(binary_warped, leftx, lefty, rightx, righty, left_fit, right_fit):
    out_img = np.dstack((binary_warped, binary_warped, binary_warped)) * 255
...
def find_curvature(left_fit):
    ...
    # print("Radius of Curvature: %.2fm" % curvature)


def plot_image(undist, binary_warped, left_fit, right_fit, Minv):
    ...
    curvature = find_curvature(left_fit)

    cv2.putText(result, 'Radius of Curvature: %.2fm' % curvature, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1,
                (255, 255, 255), 2)
...
def image_pipeline(image):
    ...
    return plot_image(image, binary_warped, left_fit, right_fit, Minv)
